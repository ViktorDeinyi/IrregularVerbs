...
public class Menus {
    ...
    public static void checkExistingUser() {
        ...
        if (Files.userIsExist(enterUserName)) {
            ...
            boolean checkWrongInput = false;
            do {
                System.out.println("\n\t[CREATE NEW (1)]\t\t[CHOOSE ANOTHER (2)]\t\t\t\t\t\t\t\t\t\t[GO BACK (0)]\t[EXIT (E)]");
                String userAnswer = sc.next();
                sc.nextLine();
                switch (userAnswer) {
                    case "1" -> Files.newFFile();
    ...
    public static void amountOfTests() {
        ...
        if (0 > cases || (activeDataMassive.length - 1) < cases) {
            System.out.print(Graphics.TEXT_YELLOW);
            System.out.println("Looks like we don't have enough verbs for this test. Please choose less." + Graphics.TEXT_RESET);
            Menus.amountOfTests();
        } else if (cases == 0) {
            examType();
        }
    ...
    public static void superQuizInfo() {
        ...
        do {
            ...
            switch (userAnswer) {
                ...
                case "1" -> Run.superQuizRun(activeDataMassive);
                case "e", "E" -> Run.exit();
                default -> {
                    checkWrongInput = true;
                    System.out.println(Graphics.wrongInput);
